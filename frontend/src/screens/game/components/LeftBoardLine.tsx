    ...
    if (cards.cardNumber === 0){
        return (
            <div onDrop={dragEnd} onDragOver={(e) => {e.preventDefault()}} style={{...BoardLineStyle.boardLine, display:"flex", flexDirection:'row', justifyContent:'flex-end', alignItems: "center"}}>
                <img onContextMenu={(e) => e.preventDefault()} draggable={false} style={{...cardStyles.card, display: "flex"}} src={`empty_spot.png`}></img>
            </div>
        )
    }
    ...
    return (
        <div onDrop={isOrigin ? () => {} : dragEnd} onDragOver={isOrigin? () => {} : (e) => {e.preventDefault()}} style={{...BoardLineStyle.boardLine, display:"flex", flexDirection:'row', justifyContent:'flex-end', alignItems: "center"}}>
            {[...pileData].reverse().map((card, idx) => (
                idx === 0 ? 
                <img onContextMenu={(e) => e.preventDefault()} draggable="true" onDragStart={dragStart} style={{...cardStyles.card, position: "relative", zIndex: 15-idx,  marginLeft: idx === 0 ? 0 : - 0.9 * (window.innerHeight / 6) + 0.9 * (window.innerHeight / 6) * 0.3 }} key={idx} src={`${card.symbol}_${card.value}.png`}></img>
                :
                <img onContextMenu={(e) => e.preventDefault()} draggable={false} style={{...cardStyles.card, position: "relative", zIndex: 15-idx,  marginLeft: idx === 0 ? 0 : - 0.9 * (window.innerHeight / 6) + 0.9 * (window.innerHeight / 6) * 0.3 }} key={idx} src={`${card.symbol}_${card.value}.png`}></img>
            ))}
        </div>
