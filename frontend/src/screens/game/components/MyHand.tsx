...
const MyHand = ({crapette, bin, draw, setOrigin, setDestination, revealDraw}: HandsProps) => {
    ...
    return (
        <div style={{...handStyles.hand}}>
            {draw.cardNumber === -1 ? <img draggable={false} onClick={revealDraw} style={cardStyles.card} src={'card_back.png'}></img> : draw.cardNumber === 0 ? <img draggable={false} style={cardStyles.card} src={'empty_spot.png'}></img> : <img draggable="true" onClick={revealDraw} onDragStart={() => dragStart("DRAW")} style={cardStyles.card} src={`${draw.cards?.[0].symbol}_${draw.cards?.[0].value}.png`}></img>}
