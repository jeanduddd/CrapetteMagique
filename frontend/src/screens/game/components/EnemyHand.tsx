...
const EnemyHand = ({crapette, bin, draw, setDestination}: HandsProps) => {
    ...
    return (
        <div style={{...handStyles.hand}}>
            ...
            {bin.cardNumber === 0 ? <img draggable={false} style={cardStyles.card} src={'trash_spot.png'}></img> : <img draggable={false} onDrop={() => dragEnd("BIN")} onDragOver={(e) => {e.preventDefault()}} style={cardStyles.card} src={`${bin.cards?.[0].symbol}_${crapette.cards?.[0].value}.png`}></img>}
            {draw.cardNumber === -1 ? <img draggable={false} style={cardStyles.card} src={'card_back.png'}></img> : draw.cardNumber === 0 ? <img draggable={false} style={cardStyles.card} src={'empty_spot.png'}></img> : <img draggable={false} style={cardStyles.card} src={`${bin.cards?.[0].symbol}_${crapette.cards?.[0].value}.png`}></img>}
        </div>
    )
}
